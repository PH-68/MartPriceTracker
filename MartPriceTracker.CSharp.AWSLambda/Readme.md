...
WORKDIR /src
COPY ["MartPriceTracker.CSharp.AWSLambda.csproj", "MartPriceTracker.CSharp.AWSLambda/"]
RUN dotnet restore "MartPriceTracker.CSharp.AWSLambda/MartPriceTracker.CSharp.AWSLambda.csproj"

WORKDIR "/src/MartPriceTracker.CSharp.AWSLambda"
COPY . .
RUN dotnet build "MartPriceTracker.CSharp.AWSLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "MartPriceTracker.CSharp.AWSLambda.csproj" \
...
Execute unit tests
```
cd "MartPriceTracker.CSharp.AWSLambda/test/MartPriceTracker.CSharp.AWSLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "MartPriceTracker.CSharp.AWSLambda/src/MartPriceTracker.CSharp.AWSLambda"
